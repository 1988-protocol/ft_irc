...
void Server::executeChannelPrivmsg()
{
	//채널 존재하는지 체크 후 없으면 401 or 403 에러코드
	//채널 존재하면, 메시지를 보내는 유저가 채널에 참여하고 있는지 확인 (외부인 차단)
	//채널 참여자 아니면 404 에러코드
	//채널 참여자라면 본인 제외 채널 유저들에게 메시지 전송
}

void Server::executeInvite()
{
	...
	//초대 list에 추가
	//초대 보낸 사람에게 성공 응답을 주고, 초대 받은 사람에게 누가 초대했는지 알림
}

void Server::executeTopic()
{
	//발신자가 이 채널의 멤버인지 확인
	//아니라면 442 에러코드
