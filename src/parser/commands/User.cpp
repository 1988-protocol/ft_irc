...
// The following Client & Server interfaces are required by User command:
//
// Client:
...
void User::execute(Server& server, Client& client, const Message& msg)
{
    ...
    {
        client.queueReply(reply(Numeric::ERR_NEEDMOREPARAMS, target, "USER :Not enough parameters"));
        return;
    }
    ...
    if (!client.isRegistered() && client.hasCorrectPassword() && !client.getNickname().empty())
    {
        ...
        client.queueReply(reply(Numeric::RPL_WELCOME, client.getNickname(),
